...
import { KnightAttributeManagerHarmony } from "../knights/HarmonyKnightAttributeManager.sol";
import { IPeasantsHarmony } from "../peasants/PeasantsHarmony.sol";
import { PeasantEffectManagerHarmony } from "../peasants/PeasantEffectManagerHarmony.sol";
...
contract QuestPartyManagerHarmony is
  Ownable,
  AccessControl,
  ReentrancyGuard,
  PeasantEffectManagerHarmony,
  QuestCooldownManager
{
  ...
  constructor(
    ...
    BadgeStorage _badge,
    KnightAttributeManagerHarmony _knightsAttributes
  ) {
    ...
    returns (IPeasantsHarmony.PeasantStats memory attributes)
  {
    (
      uint8 profession,
      uint8 rarity,
      IPeasantsHarmony.AttributeView[] memory data
    ) = IPeasantsHarmony(peasants).getData(id);

    attributes.profession = profession;
    attributes.rarity = rarity;
    attributes.level = uint8(data[3].value);
  }

  function getPeasantAttributesDC(
    uint256 id
  ) external view returns (uint8, uint8, uint8, uint8, uint8) {
    IPeasantsHarmony.PeasantStats memory test = _getPeasantAttributes(id);

    return (test.rarity, test.profession, test.talent, test.labour, test.level);
  }
  ...
  ) internal view returns (Party memory party) {
...

    {
      ...
      for (uint256 i = 0; i < numberOfItems; i++) {
        if (party.items.ids[i] >= EQUIPMENT_OFFSET) {
          ...
          require(items.balanceOf(msg.sender, party.items.ids[i]) > 0, "You don't own this equipment.");
        }
      }
    }
  }
  ...
  function validateAndUpdateParty(
    ...
    for (uint256 i = 0; i < party.peasants.length; i++) {
      if (party.peasants[i] == NULL_PEASANT) continue;
      require(
        IPeasantsHarmony(peasants).ownerOf(party.peasants[i]) == msg.sender
      );
    }
