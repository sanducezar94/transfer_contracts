...
contract GinsengTea is Ownable {
  ...
  function use(address _user, uint256[] memory _values) external {
    ...
    for (uint256 i = 0; i < _values.length; i++) {
      ...
      uint256 cooldownTimestamp = questParty.knightCooldowns(_values[i]);

      uint256 newCooldownTimestamp = cooldownTimestamp - (NFT_COOLDOWN / 2);
      questParty.setKnightCooldown(_values[i], newCooldownTimestamp);
    }
  }
}
